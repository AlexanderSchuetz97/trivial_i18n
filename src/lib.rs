...
pub fn i18n(input: TokenStream) -> TokenStream {
    ...
    while let Some(next) = token_iter.next() {
        match next {
            TokenTree::Group(_) => {
                panic!("Trying to parse language enum name, got TokenTree::Group which cant possibly be contained in a valid rust type path.");
            }
            TokenTree::Punct(png) => {
                if png.as_char() == ';' {
                    break;
    ...
    if p.as_char() != '=' {
        panic!("Trying to parse = after default language enum name, but got {}", p.as_char());
    }
    ...
    if p.as_char() != ';' {
        panic!("Trying to parse ; after language default file path, but got {}", p.as_char());
    }
    ...
    variants.insert(default_variant.clone(), Variant {
    ...
    while let Some(next) = token_iter.next() {
        ...
        let Some(TokenTree::Punct(p)) = token_iter.next() else {
            panic!("Trying to parse = after language enum name {language_name}, but got non Punct TokenTree");
        };

        if p.as_char() != '=' {
            panic!("Trying to parse = after language enum name {language_name}, but got {}", p.as_char());
        }

        let Some(TokenTree::Literal(lit)) = token_iter.next() else {
            panic!("Trying to parse language file path of language {language_name}, a literal, but got non literal.");
        };
        ...
        loop {
            let Some(TokenTree::Punct(p)) = token_iter.next() else {
                panic!("Trying to parse ; after language {variant_name} file path {variant_path}, but got non Punct TokenTree");
            };
            ...
            if p.as_char() != ',' {
                panic!("Trying to parse ; or , after language {variant_name} file path {variant_path}, but got {}", p.as_char());
            }

            let Some(TokenTree::Ident(lit)) = token_iter.next() else {
                panic!("Trying to parse fallback language name for language {language_name}, a Ident, but got non Ident TokenTree.");
            };
    ...
    for (_, variant) in variants.iter_mut() {
        ...
        let mut prop_file_reader =  BufReader::new(File::open(path).expect(format!("Failed to open file {} for language {}", variant.path, variant.name).as_str()));
    ...
    output.push_str(format!("pub struct I18NValue(&'static [&'static str; {}]);\n", variants.len()).as_str());
    ...
    for k in variants.get(&default_variant).unwrap().properties.keys() {
        ...
        for (_, value) in variants.iter() {
            let prop_val = value.properties.get(k).unwrap()
                .replace("\n", "\\n")
                .replace("\r", "\\r")
                .replace("\t", "\\t")
                .replace("\"", "\\\"")
    ...
    for variant in variants.values() {
        for fallback in &variant.fallbacks {
            if !variants.contains_key(fallback) {
                panic!("Language '{}' has fallback '{}' which does not exist.", variant.name, fallback);
            }
        }
    }
}

fn validate_all_keys_in_default_language(default_variant: &str, variants: &mut LinkedHashMap<String, Variant>) {
    ...
    for variant in variants.values() {
        for k in variant.properties.keys() {
            if !default_variant_value.properties.contains_key(k) {
                panic!("Language '{}' has a key called '{}' which does not exist in the default language '{}'. The default language must contain all keys!", variant.name, k, default_variant);
            }
        }
    }
}

fn resolve_fallbacks_properties(default_variant: &str, variants: &mut LinkedHashMap<String, Variant>) {
        ...
        'next_prop: for (k, default_value) in default_variant_value.properties.iter() {
            ...
            for fallback in &variant.fallbacks {
                if let Some(fallback_value) = cl.get(fallback).unwrap().properties.get(k) {
                    variant.properties.insert(k.to_string(), fallback_value.to_string());
                    continue 'next_prop;
                }
            }

            variant.properties.insert(k.to_string(), default_value.to_string());
        }
